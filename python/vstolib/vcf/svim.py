        ...
        for sample_id in sample_ids:
            ...
            if 'AD' in attributes.keys():
                ...
                alternate_allele_read_count = get_typed_value(value=attributes['AD'].split(',')[1],
                                                              default_value=-1,
                                                              type=int)
            if alternate_allele_read_count == -1 and 'SUPPORT' in attributes.keys():
                alternate_allele_read_count = get_typed_value(value=attributes['SUPPORT'],
                                                              default_value=-1,
                                                              type=int)
            if 'SVLEN' in attributes.keys():
                variant_size = abs(attributes['SVLEN'])
            if 'SEQS' in attributes.keys():
                variant_sequences = [str(sequence) for sequence in attributes['SEQS'].split(',')]
