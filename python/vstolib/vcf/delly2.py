...
def parse_delly2_somatic_callset(
    ...
    for row in df_vcf.to_dict('records'):
        for sample_id in sample_ids:
            ...
            if alternate_allele_read_count is None and 'PE' in attributes.keys():
                alternate_allele_read_count = get_typed_value(value=attributes['PE'], default_value=-1, type=int)
            if 'SVLEN' in attributes.keys():
                variant_size = abs(attributes['SVLEN'])
            if 'CONSENSUS' in attributes.keys():
                variant_sequences = [str(attributes['CONSENSUS'])]

            # Update chromosome_2 for 'BND'
            if variant_type in [VariantTypes.BREAKPOINT, VariantTypes.TRANSLOCATION]:
                chromosome_2 = str(attributes['CHR2'])

            # Update position_2 for 'BND'
            if variant_type in [VariantTypes.BREAKPOINT, VariantTypes.TRANSLOCATION]:
                position_2 = int(attributes['POS2'])
