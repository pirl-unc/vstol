...
/// Verifies that `VariantsList::subtract` function correctly subtracts another VariantsList object.
///
/// # Scenario
/// Two variants lists share just 1 variant.
///
/// # Expected
/// 1 variant in the subtracted variants list..
#[test]
fn subtract_variants_list_1() {
    ...
    let vl_subtracted: VariantsList = variants_list_1.subtract(
        &variants_list_2,
        1,
        100,
        true,
    ...
    assert_eq!(vl_subtracted.variants.len(), 1);
    assert_eq!(vl_subtracted.variants[0].variant_calls[0].id, "variant_call_2");
}
