...
/// Verifies that `VariantsList::compare` function correctly identifies shared and
/// prviate variants lists.
///
/// # Scenario
/// Two variants lists each with 2 variants only share 1 variant.
///
/// # Expected
/// 1 variant in the shared list, and 1 variant in each of the 2 private lists.
#[test]
fn compare_variants_list_1() {
    ...
    assert_eq!(vl_a_only.variants.len(), 1);
    assert_eq!(vl_b_only.variants.len(), 1);
}
